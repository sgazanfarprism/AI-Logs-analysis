...
class Orchestrator:
    ...
    def __init__(self):
        ...
        try:
            ...
            # Error signature cache for deduplication within execution window
            self.error_signature_cache = set()
    ...
    def run_analysis(
        ...
        try:
            ...
            has_errors = self.error_parser.has_error_level_logs(logs)
            
            if not has_errors:
                logger.info("No error-level logs detected in time range")
                result["status"] = "completed_no_errors"
                result["end_time"] = datetime.utcnow().isoformat() + "Z"
                result["message"] = "Analysis completed successfully - no error-level logs found"
                return result
            
            # Filter to error-level logs only
            error_logs = self.error_parser.filter_error_logs(logs)
            logger.info(f"Found {len(error_logs)} error-level logs out of {len(logs)} total logs")
            
            # Generate CSV file for error logs
            from utils.csv_exporter import CSVExporter
            csv_exporter = CSVExporter()
            csv_filepath = csv_exporter.export_to_csv(error_logs, execution_id)
            
            result["csv_file"] = str(csv_filepath)
            logger.info(f"Error logs exported to CSV: {csv_filepath}")
            ...
            # Deduplication: Filter out errors we've already seen
            original_error_count = len(parse_result["error_groups"])
            deduplicated_groups = []
            duplicate_count = 0
            
            for error_group in parse_result["error_groups"]:
                # Generate signature for first log in group
                if error_group.get("sample_logs"):
                    signature = self.error_parser.generate_error_signature(error_group["sample_logs"][0])
                    
                    if signature not in self.error_signature_cache:
                        # New error, add to cache and include in analysis
                        self.error_signature_cache.add(signature)
                        deduplicated_groups.append(error_group)
                    else:
                        # Duplicate error, skip
                        duplicate_count += 1
                        logger.info(f"Skipping duplicate error: {error_group.get('error_type')} from {error_group.get('service_name')}")
            
            if duplicate_count > 0:
                logger.info(f"Deduplicated {duplicate_count} error groups out of {original_error_count}")
                result["deduplication"] = {
                    "original_count": original_error_count,
                    "duplicate_count": duplicate_count,
                    "unique_count": len(deduplicated_groups)
                }
                # Update parse result with deduplicated groups
                parse_result["error_groups"] = deduplicated_groups
            
            # If all errors were duplicates, skip further processing
            if not deduplicated_groups:
                logger.info("All errors were duplicates - skipping RCA and solutions")
                result["status"] = "completed_all_duplicates"
                result["end_time"] = datetime.utcnow().isoformat() + "Z"
                result["message"] = "All detected errors were duplicates from previous analysis"
                return result
            ...
            if send_email:
                ...
                email_sent = self._execute_stage(
                    ...
                    parse_result["statistics"],
                    time_range,
                    csv_filepath  # Pass CSV file to email sender
                )
