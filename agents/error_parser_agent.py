...
class ErrorParserAgent:
    ...
    def classify_log(self, log: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message_val = log.get("message") or ""
        error_msg_val = log.get("error_message") or ""
        error_type_val = log.get("error_type") or ""
        combined_text = f"{message_val} {error_msg_val} {error_type_val}".lower()
    ...
    def _assess_severity(self, log: Dict[str, Any], category: str) -> str:
        ...
        message_val = log.get("message") or ""
        error_msg_val = log.get("error_message") or ""
        message = (message_val + " " + error_msg_val).lower()
    ...
    def has_error_level_logs(self, logs: List[Dict[str, Any]]) -> bool:
        """
        Check if any logs contain ERROR, EXCEPTION, FAILURE, or CRITICAL indicators
        
        Args:
            logs: List of log dictionaries
        
        Returns:
            True if error-level logs are found, False otherwise
        """
        error_keywords = [
            "error", "exception", "failure", "critical", "fatal",
            "failed", "crash", "panic", "emergency"
        ]
        
        for log in logs:
            # Check log level
            log_level = (log.get("log_level") or log.get("log.level") or log.get("level") or "").lower()
            if log_level in ["error", "critical", "fatal", "emergency"]:
                return True
            ...
            # Check error type
            error_type = (log.get("error_type") or log.get("error.type") or "").lower()
            if error_type and "exception" in error_type:
                return True
    ...
    def filter_error_logs(self, logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Filter and extract only error-level log entries
        
        Args:
            logs: List of all log dictionaries
        
        Returns:
            List of error-level logs only
        """
        error_keywords = [
            "error", "exception", "failure", "critical", "fatal",
            "failed", "crash", "panic", "emergency"
        ...
        logger.info(f"Filtered {len(error_logs)} error-level logs from {len(logs)} total logs")
        return error_logs
    
    def generate_error_signature(self, log: Dict[str, Any]) -> str:
        """
        Generate unique signature for error deduplication
        
        Signature is based on: error_type, service_name, normalized error_message
        ...
        error_type = log.get("error_type") or log.get("error.type") or "unknown"
        service_name = log.get("service_name") or log.get("service.name") or "unknown"
        ...
        signature_str = f"{error_type}|{service_name}|{normalized_message}"
    ...
    def _normalize_error_message(self, message: str) -> str:
        ...
        # Remove file paths
        normalized = re.sub(r'(?:/[\w.-]+)+|(?:[A-Z]:\\[\w\\.-]+)', '<PATH>', normalized)
        
        # Convert to lowercase and trim
        normalized = normalized.lower().strip()
        
        # Truncate to first 200 chars for signature
